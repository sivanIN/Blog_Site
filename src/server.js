...
app.get('/api/articles',async (req,res)=>{
  ...
  const client= await MongoClient.connect('mongodb+srv://userone:userone@ayyan.spinx.mongodb.net/my-blog?retryWrites=true&w=majority')  ;
    const db=client.db('my-blog')
    await db.collection('articles').find({}).toArray(function(err, results){
        // console.log(results);
    ...
    // withDB(async(db)=>{
    //     // var articleName=req.params.name;
    //     const articleaInfo= await db.collection('articles').findOne({name:"learn-react"})
    //     res.status(200).json(articleaInfo);
...
app.delete('/api/articles/:name/delete',(req,res)=>{
    const {username,text}=req.body;
    var articleName=req.params.name;
    ...
    withDB(async(db)=>{
        ...
        const articleaInfo= await db.collection('articles').deleteOne({name:articleName});
...
app.post('/api/articles/addarticles',(req,res)=>{
    ...
    withDB(async(db)=>{
        ...
        const articleaInfo= await db.collection('articles').insertOne(
